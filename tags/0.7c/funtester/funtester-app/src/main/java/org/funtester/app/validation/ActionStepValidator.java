...
/**
 * Validator for an {@link ActionStep}.
 * 
 * @author Thiago Delgado Pinto
 *
 */
public class ActionStepValidator implements Validator< ActionStep > {

	@Override
	public void validate(final ActionStep obj) throws Exception {

		if ( null == obj.getActionNickname() ) {
			final String msg = Messages.alt( "_ACTION_STEP_INFORM_ACTION", "Please inform an action" );
			throw new InvalidValueException( msg, "actionNickname" );
		}

		final int elementCount = obj.getElements().size();
		if ( elementCount < 1 ) {
			final String msg = Messages.alt( "_ACTION_STEP_INFORM_ELEMENT", "Please inform at least one element" );
		...
		if ( elementCount > maxElements ) {
			...
			final String msg = Messages.alt( "_ACTION_STEP_MAX_ELEMENTS",
